...
//This function create the base map accpeting one argument (layer group)
function creatEqMap(earthquakedata) {
    
    // Create an overlayMaps object to hold the earthquake layer.
    let overlayMaps = {
      "Earthquake Palces": earthquakedata
    };
  ...
  function createEQMarkers(features) {
    ...
    for (let row = 0; row < features.length; row++) {
      eqData=features[row];
      const date = new Date(eqData.properties.time);
...
d3.json(link).then(function(response) {
 ...
 earthquakeMarkers=createEQMarkers(GeoJSON);

  // Create an overlayMaps object to hold the earthquake layer.
  let overlayMaps = {
    "Earthquake Palces": earthquakeMarkers
  };
